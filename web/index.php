...
// Configure routes
$app->get('/session/:sessionId/stream/:streamId', function ($sessionId, $streamId) use ($app) {
    error_log("sessionId $sessionId");
    error_log("streamId $streamId");
    $payload = 
    $stream = $app->opentok->getStream($sessionId, $streamId);
    $json = json_encode($stream->jsonSerialize());
    error_log("Stream: $json");
    echo $sessionId;
});

$app->get('/session/:sessionId/connection/:connectionId', function ($sessionId, $connectionId) use ($app) {
    error_log("sessionId $sessionId");
    error_log("connectionId $connectionId");
    $payload = array(
        'data' => "new connection $connectionId",
        'type' => 'test'
    );
    $stream = $app->opentok->signal($sessionId, $payload, $connectionId);
    $stream = $app->opentok->signal($sessionId, $payload);
    $json = json_encode($stream->jsonSerialize());
    error_log("Stream: $json");
    echo $sessionId;
});
